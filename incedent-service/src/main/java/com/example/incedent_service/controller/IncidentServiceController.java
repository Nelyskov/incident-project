...
public class IncidentServiceController {
    ...
    public IncidentServiceController(IncidentService servce, MeterRegistry meterRegistry){
        incidentService = servce;
        this.requestCounter = Counter.builder("incident-service.requests.total")
                .description("Общее количество запросов на IncidentService")
                .tag("application", "incident-service")
                .register(meterRegistry);

        incidentCreatedCounter = Counter.builder("incident-service.incidents.created.total")
                .description("Общее количество запросов созданных инцидентов")
                .tag("application", "incident-service")
                .register(meterRegistry);
    ...
    public ResponseEntity<Map<String, Object>> getMetrics(){
        ...
        metrics.put("custom_metrics", List.of(
                "incidents.created.total",
                "incidents.updated.total",
                "incidents.errors.total",
                "incidents.processing.time"
        ));
